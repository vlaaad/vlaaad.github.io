...
(Get-Content .\reveal.md) | ForEach-Object {$_ -replace '\[vlaaad/reveal ".+"\]',"[vlaaad/reveal `"$free_version`"]"} | Set-Content .\reveal.md
(Get-Content .\reveal-pro.md) | ForEach-Object {$_ -replace 'dev.vlaaad/reveal-pro {:mvn/version ".+"',"dev.vlaaad/reveal-pro {:mvn/version `"$pro_version`""} | Set-Content .\reveal-pro.md
